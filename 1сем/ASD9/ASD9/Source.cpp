...
void Genetic_algo(int cities[V][V])
{
	...
	cout << "\n—тартова€ попул€ци€: " << endl;
	cout << "√еном попул€ции \t||вес генома\n";
	for (int i = 0; i < populationSize; i++)
	...
	while (temperature > 100 && gen <= gen_thres) {
		cout << endl << "Ћучший геном: " << population[0].gnome;
...
int main()
{
	...
	do
	{
		cout << "¬ведите количество потомков в одной попул€ции: ";
		cin >> child;
		cin.clear();
		cin.ignore(32767, '\n');
	} while (child <= 0);
	...
	do
	{
		cout << "¬ведите количество эволюций: ";
		cin >> evoCount;
		cin.clear();
		cin.ignore(32767, '\n');
	} while (evoCount <= 0);
